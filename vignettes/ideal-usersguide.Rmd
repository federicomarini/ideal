...
TODO: this section will be linked to the deployment-oriented version of the package, which will ideally have a separated repository on github to refer to.
...
For the user which is either new with the app UI/functionality, or not extensively familiar with the topic of differential expression, it is possible to obtain a small *guided tour* of the app by clicking on the respective help buttons, marked in [][]red?[][]. These trigger the start of a step-by-step guide and feature introduction, powered by the `r CRANpkg("rintrojs")` package.
